...
>
    ...
    <div
        ...
        x-cloak
    >
        <div class="bg-[#494949] rounded-2xl shadow-xl w-full max-w-4xl max-h-[75vh] md:max-h-[80vh] overflow-y-auto scrollbar-hide">

            {{-- Header --}}
            <div class="flex items-center justify-between px-6 py-5 border-b border-[#3f3f3f]">
                <div>
                    <h2 class="text-base md:text-lg font-semibold text-white tracking-wide">Discover</h2>
                    <p class="text-[10px] md:text-xs text-gray-400 mt-0.5">Find films by genre & language</p>
                </div>
                <button @click="close()"
            ...
            <div class="p-4 md:p-6 space-y-5">
                    ...
                    <div class="flex flex-wrap gap-2">
                        @foreach($genres as $genre)
                            <button
                                ...
                                class="px-3 py-1 rounded-full text-xs md:text-sm border transition">
                                {{ $genre['name'] }}
                            </button>
                        @endforeach
                    </div>
                ...
                {{-- Tombol --}}
                <div class="flex items-center justify-end gap-3 pt-2 border-t border-[#3f3f3f]">
                    <button
                        type="button"
                        @click="selectedGenres = []; selectedLanguages = []"
                        class="px-4 py-2 text-xs md:text-sm text-gray-400 hover:text-white transition">
                        Reset
                    </button>
                    <button
                        type="button"
                        @click="submitFilter()"
                        class="px-6 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs md:text-sm font-medium rounded-lg transition">
                        Search
                    </button>
                </div>
